...
class WoLFAgent():
    ...
    """
    def __init__(self, gamma=0.999, alpha=0.1, delta=0.0001, actions=None, high_delta=0.004, low_delta=0.002):
        self.alpha = alpha
        self.gamma = gamma
    ...
    def act(self, q_values=None, printu=False):
        if printu :
            print(self.pi)
    ...
    def observe(self, reward):
        ...
        self.q_values[self.last_action_id] = ((1.0 - self.alpha) * self.q_values[self.last_action_id]) \
                                             + (self.alpha * (reward + self.gamma*(self.q_values.argmax())))
    ...
    def _update_pi(self):
       ...
       self.pi_history2.append(self.pi[1])
